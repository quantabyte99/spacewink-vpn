...
class OpenvpnController extends Controller
{
    public function store(Request $request)
    {
        try {
            ...
            return back()->with('done', 'OpenVpn server created successfully');
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            if (! $server) {
                return response()->json(['error' => 'OpenVpn server not found'], 404);
            }
            ...
            return back()->with('done', 'OpenVpn server updated successfully');
        } catch (\Exception $e) {
            return back()->with('not', $e->getMessage());
        }
    }
    ...
    public function destroy($id)
    {
        try {
            ...
            return back()->with('done', 'OpenVpn server Deleted successfully');
    ...
    public function index(Request $request)
{
    try {
        ...
        return back()->with('not', 'OpenVpn server list retrieval failed');
    }
}
    ...
    public function status(Request $request, $id)
    {
        try {
            ...
            if (! $server) {
                return response()->json(['error' => 'OpenVpn not found'], 404);
            }
    
            $server->status = $request->status;
            $server->save();
    
            return response()->json(['success' => 'OpenVpn status updated successfully']);
        } catch (\Exception $e) {
