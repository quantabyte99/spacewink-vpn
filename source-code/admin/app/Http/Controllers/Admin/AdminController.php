...
class AdminController extends Controller
{
    ...
    public function update(Request $request)
    {
        try {
            ...
            return back()->with('update', 'Info Updated Successfully');
        } catch (\Exception $e) {
    ...
    public function changePassword(Request $request)
    {
        try {
            ...
            if (! Hash::check($request->current_password, $user->password)) {
                return back()->with('unchanged', 'Current password is incorrect');
            }

            $user->password = Hash::make($request->new_password);
            $user->save();

            return back()->with('changed', 'Password Updated Successfully');
        } catch (\Exception $e) {
            return back()->with('unchanged', 'Password change failed');
        }
    }
}
