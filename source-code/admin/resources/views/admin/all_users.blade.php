...
<div class="container-fluid p-0 min-vh-100 bg-light">
    ...
    <div class="row mb-4 px-3 pt-4">
        <div class="col-12 d-flex justify-content-between align-items-center">
            <h4 class="text-dark fw-bold">All Users</h4>
            <a href="" class="btn btn-primary text-dark" data-bs-toggle="modal" data-bs-target="#modalRegisterForm">Add User</a>
        </div>
    </div>
                ...
                <table class="table table-striped table-light text-dark mb-0 align-middle">
                    ...
                    <tbody>
                        ...
                        <tr class="text-center">
                            ...
                            <td>{{ $user->price }}</td>
                            <td>{{ $user->validity ?? 'None' }}</td>
                            <td>{{ $user->start_date ? \Carbon\Carbon::parse($user->start_date)->format('M d, Y') : 'None' }}</td>
                            <td>{{ $user->expired_date ? \Carbon\Carbon::parse($user->expired_date)->format('M d, Y') : 'None' }}</td>
                            <td>
                                @php
                                ...
                                @else
                                    <span class="badge bg-danger">
                                        Expired {{ abs((int) $daysLeft) }} day{{ abs((int) $daysLeft) > 1 ? 's' : '' }} ago
                                    </span>
                                @endif
                            </td>
                            ...
                            <td>
                                <a class="me-2 text-primary" href="#" onclick="openOrderModal({{ $user->id }}, '{{ $user->isPremium }}', '{{ $user->pakage_name }}','{{ $user->price }}', '{{ $user->validity }}')">
                                        <i class="fas fa-edit"></i> Premium/Free
                                    </a>
                        ...
                        @empty
                        <tr>
                            <td colspan="11" class="text-center text-danger">No Users</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content bg-light text-dark">
            <div class="modal-header">
                <h5 class="modal-title">Edit Premium/Free</h5>
                <button type="button" class="btn-close btn-close-dark" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
             <form id="editUserStatus" action="{{ route('admin.users.status', ['id' => ':id']) }}" method="POST"
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="isPremium">Add Premium</label>
